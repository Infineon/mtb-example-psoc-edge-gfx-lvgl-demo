 ...
 *
 * Local LVGL music-demo entry point. This application keeps a copy so it can
 * use local music sources/assets and CE-specific UI fixes without modifying
 * the shared LVGL middleware.
 */
...
#else
    lv_demo_music_list_create(root);
    lv_demo_music_main_create(root);
#endif
}
